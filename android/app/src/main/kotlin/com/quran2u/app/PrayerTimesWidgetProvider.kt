...
class PrayerTimesWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        for (widgetId in appWidgetIds) {
            ...
            // Highlight next prayer cell with green tint
            ...
            // to layout positions
            val layoutOrder = intArrayOf(0, 3, 1, 4, 2, 5) // fajr,sunrise,dhuhr,asr,maghrib,isha → layout positions

            val activeLayoutIdx = if (highlightIdx < layoutOrder.size) layoutOrder[highlightIdx] else 0

            for (i in rowIds.indices) {
                if (i == activeLayoutIdx) {
                    views.setInt(rowIds[i], "setBackgroundColor", Color.parseColor("#2200C853"))
                } else {
                    views.setInt(rowIds[i], "setBackgroundColor", Color.parseColor("#14FFFFFF"))
                }
            }
