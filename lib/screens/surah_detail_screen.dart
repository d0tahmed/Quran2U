...
// BUG FIX: Download progress state was stripped in v2. Restored so users

// get feedback while a download is in progress.
...
// Use *live* state at tap time (prevents double-tap weirdness).
...
// Release quickly; we only want to prevent accidental double taps.
...
// Never block PAUSE; only debounce PLAY to avoid double-toggles.
...
// Spinner only while the tap action is in-flight (prevents "stuck loading").
